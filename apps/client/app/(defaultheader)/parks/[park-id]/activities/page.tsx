    ...
    <div className='min-h-screen bg-gray-50'>
      ...
      <div className='bg-primary text-white py-20'>
        <div className='container mx-auto px-4 text-center'>
          ...
          <h1 className='text-5xl md:text-6xl font-light mb-4'>{park.name.toUpperCase()} ACTIVITIES</h1>
          ...
          <p className='text-lg max-w-4xl mx-auto mb-4'>
            {park.name} offers a diverse range of activities from exciting safaris to cultural experiences and adventure
            activities.
          </p>

          <p className='text-lg'>Below is a comprehensive list of activities organized by duration.</p>

          <p className='text-lg font-semibold mt-4'>Please enquire for availability and specials.</p>
        </div>
      </div>
      ...
      <div className='container mx-auto px-4 py-16'>
        {allActivities.activities.length === 0 ? (
          <div className='text-center py-16'>
            <h2 className='text-2xl font-light text-gray-600 mb-4'>No activities available</h2>
            <p className='text-gray-500'>We're currently updating our activity options for {park.name}.</p>
          </div>
        ) : (
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto'>
            {allActivities.activities
              ...
              .map((activity, index) => {
                ...
                return (
                  <Card key={activity.id} className='overflow-hidden pt-0 hover:shadow-lg transition-shadow duration-300'>
                    {/* Image Section */}
                    <div className='relative'>
                      <div className={cn('relative h-64 w-full', !hasImage && 'flex items-center justify-center bg-muted')}>
                        {hasImage ? (
                          <Image
                            src={activity.images[0]?.image?.original_url || ''}
                            alt={activity.images[0]?.image?.alt_text || activity.name}
                            fill
                        ...
                        {/* Duration Tier Badge */}
                        <Badge variant='default' className='absolute top-4 left-4 bg-green-700 hover:bg-green-700/90'>
                          {durationTier}
                        </Badge>
                      </div>
                    </div>
                    ...
                    <CardContent className='p-6'>
                      {/* Activity Name */}
                      <h3 className='text-lg font-bold text-center mb-2'>{activity.name}</h3>
                        ...
                        {activity.packages[0] ? (
                          <>
                            <div className='text-lg font-bold mb-1'>
                              From ₹{activity.packages[0].price_1.toLocaleString()}
                            </div>
        ...
        <Card className='mt-16 max-w-4xl mx-auto'>
          <CardHeader className='text-center'>
            <h2 className='text-2xl font-bold text-primary'>{park.name.toUpperCase()} ADVENTURES</h2>
            <p className='text-lg text-muted-foreground'>Specialists in {park.name}</p>
          </CardHeader>
          <CardContent>
