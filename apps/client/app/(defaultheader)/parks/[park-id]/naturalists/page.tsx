      ...
      <div className='container mx-auto px-4 py-10'>
        <div className='grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
          {naturalists.map((n: any) => (
            <article key={n.id} className='bg-white rounded-xl shadow-sm ring-1 ring-border overflow-hidden'>
              ...
              <div className='px-5 pb-5 pt-3 text-center'>
                <h2 className='text-lg font-semibold text-foreground'>{n.name}</h2>
                <p className='mt-2 text-sm text-muted-foreground line-clamp-3'>{n.description}</p>
                <div className='mt-5'>
                  <Link
                    href={`/contact?subject=Enquiry%20for%20Naturalist%20${encodeURIComponent(n.name)}`}
