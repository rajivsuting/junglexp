...
export default async function SouvenirDetailsPage(props: PageProps) {
  ...
  const images = souvenir.images?.sort((a: any, b: any) => (a.order ?? 0) - (b.order ?? 0)) || []
    ...
    <div className='min-h-screen'>
      ...
      <div className='bg-primary text-primary-foreground py-8'>
        <div className='container mx-auto px-4 flex items-center justify-between'>
          ...
          <Link href={`/parks/${souvenir.park?.slug}/souvenirs`} className='underline text-sm'>
            Back to souvenirs
          </Link>
        </div>
      </div>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
          ...
          <div>
            <Card className='overflow-hidden p-0'>
              <div className={cn('relative w-full bg-muted', mainImage ? 'aspect-[4/3]' : 'h-72')}>
                {mainImage ? (
                  <Image
          ...
          <div>
            <div className='flex items-center gap-3 mb-3'>
              {souvenir.quantity > 0 ? (
                <Badge className='bg-green-600 hover:bg-green-600/90'>In stock</Badge>
              ) : (
                <Badge variant='secondary'>Out of stock</Badge>
              )}
              <span className='text-sm text-muted-foreground'>SKU: {souvenir.id}</span>
            </div>
            ...
            <p className='text-muted-foreground leading-relaxed whitespace-pre-line mb-6'>{souvenir.description}</p>
