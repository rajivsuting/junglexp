...
const SortableAmenityItem = ({
  ...
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: displayAmenity.id })
  ...
  return (
    <div ref={setNodeRef} style={style} className='flex items-center justify-between p-3 rounded-md border'>
      <div className='flex items-center space-x-3'>
        ...
        <Badge variant={displayAmenity.type === 'existing' ? 'secondary' : 'default'}>
          {displayAmenity.type === 'existing' ? 'Existing' : 'New'}
        </Badge>
        <div className='flex items-center space-x-2'>
          <DynamicIcon name={displayAmenity.amenity.icon as IconName} size={16} className='text-gray-600' />
          <span className='text-sm font-medium'>{displayAmenity.amenity.label}</span>
        </div>
      </div>
      <Button type='button' variant='ghost' size='sm' onClick={() => onRemove(displayAmenity.id)} disabled={disabled}>
        <X className='h-4 w-4' />
      </Button>
    </div>
  )
  ...
  const handleAddExistingAmenities = (amenityIds: number[]) => {
    const fetchAmenitiesAndAdd = async () => {
      try {
        ...
        // @ts-ignore
        const amenitiesToAdd = allAmenities.filter(amenity => amenityIds.includes(amenity.id))
    ...
    <div className='space-y-4'>
      ...
      {amenities.length === 0 ? (
        ...
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={amenities.map(amenity => amenity.id)} strategy={verticalListSortingStrategy}>
