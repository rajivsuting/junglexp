...
const SortableSafetyFeatureItem = ({
  ...
  return (
    <div ref={setNodeRef} style={style} className='flex items-center justify-between p-3 rounded-md border'>
      <div className='flex items-center space-x-3'>
        <div {...attributes} {...listeners} className='cursor-grab active:cursor-grabbing'>
          <GripVertical className='h-4 w-4 text-gray-400' />
        </div>
        <Badge variant={displaySafetyFeature.type === 'existing' ? 'secondary' : 'default'}>
          {displaySafetyFeature.type === 'existing' ? 'Existing' : 'New'}
        </Badge>
        <div className='flex items-center space-x-2'>
          <DynamicIcon name={displaySafetyFeature.safetyFeature.icon as IconName} size={16} className='text-gray-600' />
          <span className='text-sm font-medium'>{displaySafetyFeature.safetyFeature.label}</span>
        </div>
      </div>
      <Button type='button' variant='ghost' size='sm' onClick={() => onRemove(displaySafetyFeature.id)} disabled={disabled}>
        <X className='h-4 w-4' />
      </Button>
    </div>
  )
...
export const SafetyFeatureManager = ({
  ...
  const handleAddExistingSafetyFeatures = (safetyFeatureIds: number[]) => {
    const fetchSafetyFeaturesAndAdd = async () => {
      try {
        ...
        // @ts-ignore
        const safetyFeaturesToAdd = allSafetyFeatures.filter(safetyFeature => safetyFeatureIds.includes(safetyFeature.id))

        // Add to display safety features with current max order + 1
        const maxOrder = Math.max(0, ...safetyFeatures.map(dsf => dsf.order))
        const newDisplaySafetyFeatures = safetyFeaturesToAdd.map((safetyFeature, index) => ({
        ...
        const updatedSafetyFeatures = [...safetyFeatures, ...newDisplaySafetyFeatures]
        // @ts-ignore
        onSafetyFeaturesChange(updatedSafetyFeatures)

        // Update safety feature IDs
        const currentSafetyFeatureIds = safetyFeatures.map(sf => sf.safetyFeature.id)
        const newSafetyFeatureIds = safetyFeaturesToAdd.map(sf => sf.id)
        // @ts-ignore
        onSafetyFeatureIdsChange([...currentSafetyFeatureIds, ...newSafetyFeatureIds])
      } catch (error) {
  ...
  const handleNewSafetyFeaturesCreated = (newSafetyFeatures: TSaftyFeatureBase[]) => {
    // Add new safety features to display list
    const maxOrder = Math.max(0, ...safetyFeatures.map(dsf => dsf.order))
    const newDisplaySafetyFeatures = newSafetyFeatures.map((safetyFeature, index) => ({
  ...
  const handleRemoveSafetyFeature = (id: string) => {
    const updatedSafetyFeatures = safetyFeatures.filter(safetyFeature => safetyFeature.id !== id)
    onSafetyFeaturesChange(updatedSafetyFeatures)

    // Update safety feature IDs
    const updatedSafetyFeatureIds = updatedSafetyFeatures.map(sf => sf.safetyFeature.id)
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over?.id) {
      const oldIndex = safetyFeatures.findIndex(item => item.id === active.id)
      ...
      // Update order values
      const updatedSafetyFeatures = newSafetyFeatures.map((safetyFeature, index) => ({
        ...safetyFeature,
        order: index + 1,
      }))
      ...
      const orderedSafetyFeatureIds = updatedSafetyFeatures.map(sf => sf.safetyFeature.id)
  ...
  return (
    <div className='space-y-4'>
      ...
      <AddExistingSafetyFeaturesModal
        ...
        selectedSafetyFeatureIds={safetyFeatures.map(sf => sf.safetyFeature.id)}
