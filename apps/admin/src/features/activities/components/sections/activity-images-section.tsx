...
import { updateActivityImages } from "@repo/actions/activities.actions";
...
interface ActivityImagesSectionProps {
...
export const ActivityImagesSection = ({
  ...
  const handleSave = async (data: ImagesFormData) => {
    ...
    if (!activityId) {
      toast.error("Activity ID is required to save images");
      return;
    }
    ...
    try {
      ...
      const imagesToDelete = [...initialImageIds].filter(
        (id: number) => !currentImageIds.has(id)
      );
      ...
      // 5) Update activity images (handles create, update, delete, reorder)
      await updateActivityImages(Number(activityId), final);
