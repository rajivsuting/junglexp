...
export default function RoomPlansSection({ initialData, roomId, onSave }: RoomPlansSectionProps) {
  ...
  return (
    <div className='space-y-6'>
      <div>
        ...
        <p className='text-sm text-muted-foreground'>Manage different meal plans and pricing for this room</p>
      </div>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className='max-w-2xl'>
          <DialogHeader>
            <DialogTitle>{editingPlan ? 'Edit Room Plan' : 'Add New Room Plan'}</DialogTitle>
          </DialogHeader>
              ...
              <div key={plan.id} className='flex items-center justify-between p-4 border rounded-lg'>
                <div className='flex-1'>
                  <div className='flex items-center gap-4'>
                    <div className='font-medium'>{mealPlanOptions.find(opt => opt.value === plan.plan_type)?.label}</div>
                ...
                <div className='flex items-center gap-2'>
                  <Button variant='outline' size='sm' onClick={() => openModal(plan)} disabled={isLoading}>
                    Edit
                  </Button>
