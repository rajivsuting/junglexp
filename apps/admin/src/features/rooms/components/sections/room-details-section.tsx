...
export default function RoomDetailsSection({ initialData, onSuccess }: RoomDetailsSectionProps) {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder='Enter room name' {...field} disabled={isLoading} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
            ...
            render={({ field }) => (
              <FormItem className='w-full max-w-[400px]'>
                ...
                <Select
                  ...
                  <SelectContent>
                    {hotels.map(hotel => (
                      <SelectItem className='whitespace-normal break-words' key={hotel.id} value={hotel.id.toString()}>
                        {hotel.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea placeholder='Enter room description' className='min-h-[100px]' {...field} disabled={isLoading} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <div className='flex items-center gap-4'>
          ...
          {!isEditMode && (
            <Button type='button' variant='outline' onClick={() => router.push('/rooms')} disabled={isLoading}>
              Cancel
            </Button>
          )}
        </div>
