...
export const getReels = async (filters: TGetReelsFilters) => {
  ...
  const where = and(search ? ilike(Reels.title, `%${search}%`) : undefined, status ? eq(Reels.status, status) : undefined)
  ...
  const totalResponse = await db.select({ value: count() }).from(Reels).where(where)
...
export const updateReel = async (id: string, values: Partial<TNewReel>) => {
  const updated = await db.update(Reels).set(values).where(eq(Reels.id, id)).returning()
  getReelsFromCache()
  return updated[0] ?? null
}

export const setReelStatus = async (id: string, status: TReelStatus) => {
  const updated = await db.update(Reels).set({ status }).where(eq(Reels.id, id)).returning()
  getReelsFromCache()
  return updated[0] ?? null
}
