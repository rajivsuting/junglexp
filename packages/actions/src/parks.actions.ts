...
import { NationalParks, nationaParkInsertSchema, ParkImages, parkImagesInsertSchema } from '@repo/db/schema/park'

import { deleteImages } from './image.actions'
import { getOrSet } from './libs/cache'
import { nationalParkBySlugKey } from './libs/keys'

import type { TNewNationalPark, TNationalPark, TNationalParkBase } from '@repo/db'
...
export const deleteNationalPark = async (parkId: string) => {
  ...
  await db.delete(schema.NationalParks).where(eq(schema.NationalParks.id, Number(parkId)))
}
...
const upsertImages = async (parkId: number, isMobile: boolean, imagesPayload: CreateParkImagesPayload) => {
  if (!db) throw new Error('Database connection not available')

  // 2) Insert new Images for any 'added' entries that include full image payload
  const toInsert = imagesPayload.added.filter((a): a is { image: NewImageInput; order: number } => 'image' in a)
  ...
  let allForJoin = [...existingPairs, ...addedExistingIdPairs, ...addedNewPairs].filter(x => !removedSet.has(x.id as number))
  ...
  if (imagesPayload.removed.length > 0) {
    await db
      .delete(ParkImages)
      .where(and(eq(ParkImages.park_id, parkId), inArray(ParkImages.image_id, imagesPayload.removed)))
  ...
  //   - Else -> insert
  const currentJoins = await db.select().from(ParkImages).where(eq(ParkImages.park_id, parkId))
  ...
  for (const u of toUpdate) {
    await db
      .update(ParkImages)
      .set({ order: u.order })
      .where(and(eq(ParkImages.park_id, parkId), eq(ParkImages.image_id, u.image_id)))
  }

  // Update alt text for existing images if provided
  const altTextUpdates = imagesPayload.existing.filter(existing => existing.alt_text !== undefined)
  ...
  if (finalImageIds.length > 0) {
    const imgs = await db.select().from(Images).where(inArray(Images.id, finalImageIds))
    for (const img of imgs) imagesMap.set(img.id, img)
  }

  // Re-read joins to ensure we reflect the latest order/state
  const finalJoins = await db.select().from(ParkImages).where(eq(ParkImages.park_id, parkId))
...
export const getNationalParks = async (filters: TGetNationalParksFilters = {}) => {
  ...
  const totalResponse = await db!.select({ count: count() }).from(NationalParks).where(where)

  const destinations = await db!.query.NationalParks.findMany({
    where,
    limit: filters.limit,
    offset: filters.page && filters.limit ? (filters.page - 1) * filters.limit : 0,
    with: {
      city: {
        with: { state: true },
      },
...
export const createPark = async (
  ...
  if (parkId) {
    ...
    // @ts-ignore
    const [createdPark] = await db.insert(NationalParks).values(parsed).returning()
  ...
  const mobileResponseImages = await upsertImages(park.id, true, mobileImagesPayload)
  return {
    park: {
      ...park,
...
export const updateParkImages = async (
  ...
  const existing = await db.select().from(ParkImages).where(eq(ParkImages.park_id, parkId))
  ...
  if (imagesToDelete.length > 0) {
    operations.push(
      db.delete(ParkImages).where(and(eq(ParkImages.park_id, parkId), inArray(ParkImages.image_id, imagesToDelete)))
    )
  }
  ...
  for (const img of imagesToUpdate) {
    if (img.image_id !== null) {
      ...
      if (updateData && img.order !== updateData.order) {
        operations.push(db.update(ParkImages).set({ order: updateData.order }).where(eq(ParkImages.id, img.id)))
      }
    }
  }
  ...
  return await db!.select().from(ParkImages).where(eq(ParkImages.park_id, parkId)).orderBy(ParkImages.order)
}
