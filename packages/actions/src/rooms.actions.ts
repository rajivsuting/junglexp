...
import type { TNewRoom, TNewRoomImage, TNewRoomAmenity, TNewRoomPlan } from '@repo/db/schema/rooms'
...
export const getRooms = async (filters: TGetRoomsFilters) => {
  ...
  const where = and(searchConditions.length > 0 ? or(...searchConditions) : undefined, hotelCondition)
...
export const updateRoom = async (roomId: number, payload: Partial<TNewRoom>) => {
  const parsed = roomsInsertSchema.partial().parse(payload)
  // @ts-ignore
  const [updatedRoom] = await db.update(schema.Rooms).set(parsed).where(eq(schema.Rooms.id, roomId)).returning()
...
export const deleteRoom = async (roomId: number) => {
  const [deletedRoom] = await db.delete(schema.Rooms).where(eq(schema.Rooms.id, roomId)).returning()
...
export const addRoomImages = async (roomId: number, imageIds: number[]) => {
  ...
  // @ts-ignore
  const insertedImages = await db.insert(RoomImages).values(imagesToInsert).returning()
...
export const updateRoomImages = async (
  ...
  const existing = await db.select().from(RoomImages).where(eq(RoomImages.room_id, roomId))
  ...
  if (imagesToDelete.length > 0) {
    operations.push(
      db.delete(RoomImages).where(and(eq(RoomImages.room_id, roomId), inArray(RoomImages.image_id, imagesToDelete)))
    )
  }
  ...
  for (const img of imagesToUpdate) {
    if (img.image_id !== null) {
      ...
      if (updateData && img.order !== updateData.order) {
        operations.push(db.update(RoomImages).set({ order: updateData.order }).where(eq(RoomImages.id, img.id)))
      }
    }
  }
  ...
  if (imageAltTextUpdates.length > 0) {
    const altTextOperations = imageAltTextUpdates.map(update =>
      db.update(schema.Images).set({ alt_text: update.alt_text }).where(eq(schema.Images.id, update.image_id))
    )
    await Promise.all(altTextOperations)
  }

  // Return updated room images
  return await db.select().from(RoomImages).where(eq(RoomImages.room_id, roomId)).orderBy(RoomImages.order)
}

// Room Amenities
export const addRoomAmenities = async (roomId: number, amenityIds: number[]) => {
  ...
  // @ts-ignore
  const insertedAmenities = await db.insert(RoomAmenities).values(amenitiesToInsert).returning()

  return insertedAmenities
}

export const removeRoomAmenities = async (roomId: number, amenityIds: number[]) => {
...
export const updateRoomAmenities = async (roomId: number, orderedAmenityIds: number[]) => {
  // Fetch existing amenities for the room
  const existing = await db.select().from(RoomAmenities).where(eq(RoomAmenities.room_id, roomId))

  const existingAmenityIds = existing.map(ra => ra.amenity_id).filter((id): id is number => id !== null)

  // Determine deletions and creations
  const toDelete = existingAmenityIds.filter(id => !orderedAmenityIds.includes(id))
  const toCreate = orderedAmenityIds.filter(id => !existingAmenityIds.includes(id))

  const ops: Promise<unknown>[] = []

  if (toDelete.length > 0) {
    ops.push(
      db.delete(RoomAmenities).where(and(eq(RoomAmenities.room_id, roomId), inArray(RoomAmenities.amenity_id, toDelete)))
    )
  }
  ...
  for (const item of existing) {
    ...
    if (item.order !== newIndex) {
      ops.push(db.update(RoomAmenities).set({ order: newIndex }).where(eq(RoomAmenities.id, item.id)))
    }
  }
  ...
  // Return updated list
  return await db.select().from(RoomAmenities).where(eq(RoomAmenities.room_id, roomId)).orderBy(RoomAmenities.order)
}
...
export const updateRoomPlan = async (planId: number, payload: Partial<TNewRoomPlan>) => {
  const parsed = roomPlansInsertSchema.partial().parse(payload)
  // @ts-ignore
  const [updatedPlan] = await db.update(RoomPlans).set(parsed).where(eq(RoomPlans.id, planId)).returning()
...
export const deleteRoomPlan = async (planId: number) => {
  const [deletedPlan] = await db.delete(RoomPlans).where(eq(RoomPlans.id, planId)).returning()
