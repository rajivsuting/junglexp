...
export const getZones = async (filters: TGetZonesFilters) => {
  ...
  const totalResponse = await db.select({ count: count() }).from(Zones).where(where)
...
export const updateZone = async (zoneId: number, payload: Partial<TNewZone>) => {
  const parsed = zonesInsertSchema.partial().parse(payload)
  // @ts-ignore
  const [updatedZone] = await db.update(schema.Zones).set(parsed).where(eq(schema.Zones.id, zoneId)).returning()
...
export const deleteZone = async (zoneId: number) => {
  const [deletedZone] = await db.delete(schema.Zones).where(eq(schema.Zones.id, zoneId)).returning()
