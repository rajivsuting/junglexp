...
export const getActivities = async (filters: TGetActivitiesFilters) => {
  ...
  const totalResponse = await db.select({ count: count() }).from(Activities).where(where)
...
export const getActivitiesByParkSlug = async (filters: TGetActivitiesByParkSlugFilters) => {
  if (!db) return { activities: [], total: 0, totalPages: 0 }
  ...
  // Ensure page and limit are valid numbers and calculate safe offset
  const page = filters.page !== undefined ? Math.max(1, filters.page) : undefined
  const limit = filters.limit !== undefined ? Math.max(1, filters.limit) : undefined
...
export const updateActivity = async (id: number, data: Partial<TNewActivity>) => {
  if (!db) throw new Error('Database connection not available')
...
export const updateActivityImage = async (id: number, data: Partial<TNewActivityImage>) => {
  if (!db) throw new Error('Database connection not available')

  const [image] = await db.update(ActivityImages).set(data).where(eq(ActivityImages.id, id)).returning()
  return image
}
...
export const updateActivityImages = async (
  ...
  const existing = await db.select().from(ActivityImages).where(eq(ActivityImages.activity_id, activityId))

  const existingImageIds = existing.filter(img => img.image_id !== null).map(img => img.image_id as number)
  ...
  if (imagesToDelete.length > 0 && db) {
    operations.push(
      db!
        ...
        .where(and(eq(ActivityImages.activity_id, activityId), inArray(ActivityImages.image_id, imagesToDelete)))
    )
  }
  ...
  const imagesToUpdate = existing.filter(img => img.image_id !== null && newImageIds.includes(img.image_id))
  ...
  for (const img of imagesToUpdate) {
    if (img.image_id !== null && db) {
      ...
      if (updateData && img.order !== updateData.order) {
        operations.push(db!.update(ActivityImages).set({ order: updateData.order }).where(eq(ActivityImages.id, img.id)))
      }
    }
  }
  ...
  const imageAltTextUpdates = imageUpdates.filter(update => update.alt_text !== undefined)
  if (imageAltTextUpdates.length > 0 && db) {
    const altTextOperations = imageAltTextUpdates.map(update =>
      db!.update(Images).set({ alt_text: update.alt_text }).where(eq(Images.id, update.image_id))
    )
...
export const createActivityItinerary = async (data: TNewActivityItinerary) => {
  ...
  const [itinerary] = await db.insert(ActivityItinerary).values(data).returning()
  return itinerary
}

export const updateActivityItinerary = async (id: number, data: Partial<TNewActivityItinerary>) => {
  if (!db) throw new Error('Database connection not available')

  const [itinerary] = await db.update(ActivityItinerary).set(data).where(eq(ActivityItinerary.id, id)).returning()
  return itinerary
}
...
export const updateActivityItineraries = async (
  ...
  const incomingIds = new Set(itineraryItems.filter(i => i.id !== undefined).map(i => i.id as number))
  ...
  if (idsToDelete.length > 0) {
    operations.push(db.delete(ActivityItinerary).where(inArray(ActivityItinerary.id, idsToDelete)))
  }
  ...
  const itemsToUpdate = itineraryItems.filter(i => i.id !== undefined) as Array<{
    id: number
    title: string
    description: string
    order: number
  ...
  for (const incoming of itemsToUpdate) {
    ...
    const needsUpdate =
      current.title !== incoming.title || current.description !== incoming.description || current.order !== incoming.order
...
export const updateActivityAmenity = async (id: number, data: Partial<TNewActivityAmenity>) => {
  if (!db) throw new Error('Database connection not available')

  const [amenity] = await db.update(ActivityAmenities).set(data).where(eq(ActivityAmenities.id, id)).returning()
  return amenity
}
...
export const createActivityPackage = async (data: TCreateActivityPackage) => {
  ...
  const nextOrder = existingPackages.length > 0 ? Math.max(...existingPackages.map(p => p.order)) + 1 : 0
  ...
  const [package_] = await db.insert(ActivityPackages).values(packageData).returning()
  return package_
}

export const updateActivityPackage = async (id: number, data: Partial<TNewActivityPackage>) => {
  if (!db) throw new Error('Database connection not available')

  const [package_] = await db.update(ActivityPackages).set(data).where(eq(ActivityPackages.id, id)).returning()
  return package_
}
...
export const updateActivityPackages = async (
  ...
  const existingIds = new Set(existing.map(e => e.id))
  const incomingIds = new Set(packageItems.filter(i => i.id !== undefined).map(i => i.id as number))
  ...
  const idsToDelete = [...existingIds].filter(id => !incomingIds.has(id))
  if (idsToDelete.length > 0) {
    operations.push(db.delete(ActivityPackages).where(inArray(ActivityPackages.id, idsToDelete)))
  }
  ...
  const itemsToUpdate = packageItems.filter(i => i.id !== undefined) as Array<{
    id: number
    name: string
    duration: number
    number: number
...
export const updateActivityAmenities = async (activityId: number, amenityIds: number[]) => {
  if (!db) throw new Error('Database connection not available')

  // Get existing activity amenities
  const existing = await db.select().from(ActivityAmenities).where(eq(ActivityAmenities.activity_id, activityId))
  ...
  const amenitiesToCreate = amenityIds.filter(amenityId => !existingAmenityIds.includes(amenityId))

  // Find amenities to delete (existing amenity IDs not in new list)
  const amenitiesToDelete = existingAmenityIds.filter(existingAmenityId => !amenityIds.includes(existingAmenityId))
  ...
  if (amenitiesToDelete.length > 0) {
    const deleteOperation = db
      ...
      .where(and(eq(ActivityAmenities.activity_id, activityId), inArray(ActivityAmenities.amenity_id, amenitiesToDelete)))
  ...
  const amenitiesToUpdate = existing.filter(
    amenity => amenity.amenity_id !== null && amenityIds.includes(amenity.amenity_id)
  )
...
export const updateActivityPolicies = async (activityId: number, kind: 'include' | 'exclude', policyIds: number[]) => {
  if (!db) throw new Error('Database connection not available')
  ...
  const existing = await db
    ...
    .where(and(eq(ActivityPolicies.activity_id, activityId), eq(Policies.kind, kind)))

  const existingPolicyIds = existing.filter(policy => policy.policy_id !== null).map(policy => policy.policy_id as number)

  // Find policies to create (new policy IDs not in existing)
  const policiesToCreate = policyIds.filter(policyId => !existingPolicyIds.includes(policyId))

  // Find policies to delete (existing policy IDs not in new list)
  const policiesToDelete = existingPolicyIds.filter(existingPolicyId => !policyIds.includes(existingPolicyId))
  ...
  if (policiesToDelete.length > 0) {
    const deleteOperation = db
      ...
      .where(and(eq(ActivityPolicies.activity_id, activityId), inArray(ActivityPolicies.policy_id, policiesToDelete)))
    operations.push(deleteOperation)
  }
  ...
  const policiesToUpdate = existing.filter(policy => policy.policy_id !== null && policyIds.includes(policy.policy_id))
...
export const getPackagesByActivityId = async (activityId: string) => {
  ...
  return await db!.query.ActivityPackages.findMany({
    where: and(eq(ActivityPackages.activity_id, Number(activityId)), eq(ActivityPackages.active, true)),
  })
}
