...
export const createSafetyFeatures = async (data: TNewSaftyFeature[]) => {
  ...
  const parsedSafetyFeatures = data.map(feature => insertSaftyFeaturesSchema.parse(feature))
  // @ts-ignore
  const results = await db.insert(SaftyFeatures).values(parsedSafetyFeatures).returning()
