...
export const roomsInsertSchema = createInsertSchema(Rooms, {
  ...
  description: (schema) =>
    schema.description.min(1, "Room description is required"),
  room_qty: (schema) =>
    schema.room_qty.min(1, "Room quantity must be at least 1"),
  capacity: (schema) =>
    schema.capacity.min(1, "Room capacity must be at least 1"),
});
